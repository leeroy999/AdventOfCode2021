...
function part1()
    bags = {}
    function create_bags(str) -- for creating all the bags in key-value pairs
        local key = string.match(str, "%a+%s%a+%sbag")
        local i = 1
        bags[key] = {}
        for color in string.gmatch(str, "%a+%s%a+%sbag") do
            if color ~= key then
                bags[key][i] = color
                i = i + 1
            end
            if color == "shiny gold bag" then
                bags[key] = 1
                break
            end
            if color == "no other bag" then
                bags[key] = 0
                break
            end
        end
    end
    for i = 1, #arr do --create bag for each element in (arr)
        if string.match(arr[i], "%a+%s%a+%sbag") ~= "shiny gold bag" then
            create_bags(arr[i])
        end
    end
    function check_gold(color_bag) -- checks if bag has gold in it
        local color_arr = bags[color_bag]
        if color_arr == 1 then 
            return true
        elseif color_arr == 0 then
            return false
        else
            local count = 0
            for i = 1, #color_arr do
                if check_gold(color_arr[i]) then
                    count = count + 1
                end 
            end
            if count >= 1 then
    ...
    --]]
    --execute counting of number of bags with shiny gold
    local count_bags = 0
    for k,v in pairs(bags) do
        print(k)
        if check_gold(k) == true then
...
function part2()
    ...
    function create_bags(str) -- for creating all the bags in key-value pairs
        ...
        local i = 1
        bags[key] = {}
        if string.match(str, "%d%s%a+%s%a+%sbag") == nil then
            bags[key] = 0
        else
            for color in string.gmatch(str, "%d%s%a+%s%a+%sbag") do
                bags[key][i] = color
                i = i + 1
            end
        end
...
--[[
    ...
    function count_bags(color)
        if bags[color] == 0 then
            print(color .. ":" .. 1)
            return 1
        else
            local count = 1
            local color_arr = bags[color]
            for i = 1, #color_arr do
                number_bags = tonumber(string.sub(color_arr[i], 1, 1))
                count = count + (number_bags * count_bags(string.sub(color_arr[i], 3)))
            end
            print(color .. ":" .. tostring(count))
            return count
        end
    end
    --execute counting of number of bags in shiny gold
